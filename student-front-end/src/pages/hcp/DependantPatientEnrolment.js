...
const DependantPatientEnrolment = () => {
    ...
    useEffect(() => {
        ...
        if (personnelNumber) {
            fetchDependantPatient(personnelNumber, suffix);
        }
    }, [personnelNumber]);

    const fetchDependantPatient = async (personnelNumber, suffix) => {
        try {
            const response = await axios.get(BASE_URL + `api/v1/patients/dependants/${personnelNumber}/${suffix}`);
    ...
    const enrolPatient = () => {
        ...
        const requestBody = {
            id: uniqueID,
            personnelNumberPlusSuffix: personnelNumber + '-' + suffix,
        ...
        if (firstFingerPrintData !== '' && secondFingerPrintData !== '' && thirdFingerPrintData !== '') {
            axios.post(BASE_URL + `api/v1/fingerprints/create`, requestBody).then(res => {
                if (res.status === 200) {
                    ...
                    AuditTrailService.createLog("successful dependant member enrolment");
                }
            }).catch(
                err => {
    ...
    return (
        <DashboardLayout>
            <div className="p-4">
                <h1 className="text-2xl font-bold mb-6">Dependant Biometric Enrolment</h1>
                <div className="bg-gray-200 p-4" style={{ borderRadius: '20px', padding: '30px' }}>
                    <div className="grid grid-cols-2 gap-1" > {/* Reduced gap to 2 */}
                        ...
                        <p className="text-lg" style={{ fontStyle: 'italic', color: enrolmentStatus === 'NOT ENROLLED' ? 'red' : 'green' }}> {enrolmentStatus} </p>
                    </div>
...
export default DependantPatientEnrolment;