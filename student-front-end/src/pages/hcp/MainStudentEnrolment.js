...
const MainStudentEnrolment = () => {
    ...
    const enrolPatient = () => {
        ...
        if (firstFingerPrintData !== '' && secondFingerPrintData !== '' && thirdFingerPrintData !== '') {
            axios.post(BASE_URL + `api/v1/fingerprints/update`, requestBody).then(async res => {
                if (res.status === 200) {
                    toast.success("successful student enrolment ");
                    AuditTrailService.createLog("successful student enrolment : " + membership);
                    await fetchPrincipalFingerPrintsData();
                    setEnrolmentStatus('ENROLLED');
                    setFingerPrintsArray([])
                ...
                <div className="bg-gray-200 p-4" style={{ borderRadius: '20px', padding: '30px' }}>
                    ...
                    <div className="flex">
                        <div className="flex flex-col items-start flex-1">
                            ...
                            <div className="text-lg font-bold mb-3">Enrolment Status: <span style={{ fontStyle: 'italic', color: enrolmentStatus !== 'ENROLLED' ? 'red' : 'green' }}>
                                {enrolmentStatus !== 'ENROLLED' ? 'NOT ENROLLED' : enrolmentStatus}</span></div>
                        </div>
                        <div className="flex flex-col items-start flex-1">
