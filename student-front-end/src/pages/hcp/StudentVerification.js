...
const StudentVerification = () => {
    ...
    const [accessPoints, setAccessPoints] = useState([]); // 🔹 store all access points
    const [accessPoint, setAccessPoint] = useState(""); // 🔹 selected access point
    const itemsPerPage = 20;
    const navigate = useNavigate();
    const location = useLocation();

    // 🔹 Fetch available access points from API
    const fetchAccessPoints = async () => {
        try {
            ...
            // If access point is passed via navigation, use it; else default to first one
            if (location.state && location.state.accessPoint) {
                setAccessPoint(location.state.accessPoint);
            } else if (data.length > 0) {
                setAccessPoint(data[0].name || data[0].accessPointName);
            }
        } catch (error) {
                    ...
                    >
                        {accessPoints.length > 0 ? (
                            accessPoints.map((point) => (
                                <option
                                    key={point.id}
                                    value={point.name || point.accessPointName}
                                >
                                    {point.name || point.accessPointName}
                                </option>
                            ))
                        ) : (
